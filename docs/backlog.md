...
| `BL-M1-03` | Giỏ offline và đặt món: `ordering/`, cart, order pages | `FR-CUS-06`, `FR-CUS-08`, `FR-CUS-09`, `TM-ORD-01`…`03` | Client không gửi giá; idempotent; order+outbox nguyên tử; test tampering xanh | READY |
| `BL-M1-04` | KDS realtime: `web-staff/app/kds`, WebSocket resume/dedupe | `FR-BAR-01`…`04`, `FR-BAR-06`, `EC-06`, `TM-EVT-01` | Đơn tới KDS ≤1s; optimistic lock; reconnect/resume không mất/lặp hiệu ứng | LATER |
| `BL-M1-05` | Hết nguyên liệu lan toả: `inventory/`, catalog event, KDS | `FR-BAR-05`, `EC-04` | Báo hết → ẩn món ≤2s; đơn đang chờ được cảnh báo | LATER |
| `BL-M1-06` | Cổng thoát M1 trên staging | `FR-CUS-01`…`11`, `FR-BAR-01`…`06` | Một đơn đi từ QR tới `SERVED`; Chromium/WebKit; k6 và test security liên quan xanh | LATER |
...
| `OPEN-02` | Fail-open/fail-closed khi Redis lỗi theo từng endpoint | `BL-M1-01`, `TM-ORD-03`, `RR-04` — không còn ảnh hưởng `BL-M0-06`: idempotency đã chốt chỉ dùng PostgreSQL | Security + Architecture chốt trước test tải M1 |
...
| `OPEN-10` | `storeId` chưa vào MDC log (`NFR-OBS-03`): JWT staff/admin mang một danh sách chi nhánh (claim `stores`), không phải một chi nhánh đang hoạt động cho request hiện tại — cần quyết định UX (header riêng? tham số đường dẫn?) trước khi biết lấy giá trị nào đặt vào MDC | `shared/security`, `web-staff` | Product + Architecture chốt cách xác định chi nhánh đang hoạt động của một request trước `BL-M1-01` |
...
| `OPEN-13` | Hai workflow chưa chạy được thật trên GitHub: workspace chính không có metadata `.git` (đã ghi từ `BL-M0-01`). `security.yml` chỉ được kiểm bằng actionlint (hợp lệ cú pháp) và các bước riêng lẻ (build ảnh Docker, sinh SBOM) chạy tay cục bộ — chưa có lượt chạy CI thật nào xác nhận cổng chặn (secret/CVSS/image finding) hoạt động đúng | `.github/workflows/*.yml` | Gắn lại Git remote rồi chạy thật trên GitHub trước khi coi `BL-M0-12`/`BL-M0-03` là đã kiểm chứng đầy đủ trên CI thật |
...
| `OPEN-15` | `TM-SES-01` đòi token phiên bàn gắn với `deviceId` ("token khác thiết bị bị từ chối") nhưng `BL-M1-01` chỉ gắn thiết bị ở tầng dữ liệu (`session_device`), token thì bất kỳ thiết bị nào giữ được cũng dùng được — đánh cắp/chia sẻ token vẫn đọc/đặt món được cho bàn đó. Cần thêm claim `did` vào token khách, một tiêu đề định danh thiết bị (`X-Device-Id`?) trên mọi request vùng khách, và một filter đối chiếu hai giá trị — đây là thay đổi hợp đồng, phải sửa `openapi.yaml` trước | `venue`, `shared/security`, `web-guest` | Chốt tên header/claim rồi làm trước khi coi `TM-SES-01` đã có mitigation thật, không chỉ dữ liệu |
| `OPEN-16` | `NFR-PERF-07` (≤ 180 KB gzip gói JS ban đầu của `web-guest`) chưa được đo bằng bundle analyzer thật — Turbopack production build không in bảng "First Load JS" theo route như build cũ. Cộng thô gzip của mọi chunk chia sẻ (Next 16 + React 19) đã ra khoảng 176 KB, sát trần dù chưa cộng JS riêng của từng trang — rủi ro thật, không phải lo xa | `web-guest` | Thêm bundle analyzer (`@next/bundle-analyzer` hoặc tương đương) và cổng chặn CI trước khi coi `NFR-PERF-07` đã kiểm chứng |
...
| `OPEN-18` | `BL-M1-03` mới xong phần backend (`ordering/`) — giỏ hàng offline lưu IndexedDB (`FR-CUS-06`) và trang đặt món phía `web-guest` **chưa có dòng code nào**. Thẻ chưa `DONE` vì thiếu nửa "lát dọc" | `web-guest` | Dựng `app/(session)/cart` + tích hợp `IndexedDB`, gọi `POST /guest/orders` với `Idempotency-Key` sinh phía client, trước khi coi `BL-M1-03` xong |
